...
public class GameView extends SurfaceView implements Runnable {
     ...
     * @param scoreHandler
     */
    public GameView(Context context, boolean isMultiplayer, boolean isP1, boolean isSplitScreen, int windowHeight, int windowWidth, int myPlayerSkinImageID, GameScreenActivity.ScoreHandler scoreHandler) {
        super(context);
    ...
    public boolean onTouchEvent(MotionEvent event) {//כל עוד האצבע על המסך ימשיך לעקוב אחרי האצבע ולזוז אחריה אבל כשמרימים את האצבע הבלוק ייעצר
        if(!isSplitScreen){
            ...
            for(int i = 0; i < pointerCount; i++){
                if (event.getY(i) > windowHeight / 2) {
                    myBlock.setXTarget(event.getX(i));
                } else {
                    enemyBlock.setxTarget(event.getX(i));
                }
...
//            }
//            else {
//                enemyBlock.setxTarget(event.getX());
//                switch (event.getAction() & MotionEvent.ACTION_MASK) {
//
//                    case MotionEvent.ACTION_UP:
//                        enemyBlock.setxTarget(enemyBlock.getXPos());
//                        break;
//                }
//            }
        }
