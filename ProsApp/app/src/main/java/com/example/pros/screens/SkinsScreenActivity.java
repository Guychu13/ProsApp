...
public class SkinsScreenActivity extends AppCompatActivity implements SkinsRecyclerAdapter.ItemClickListener {
    ...
    public void onClick(int position) {
        if(allSkins.get(position).getUnlocked()){
            User.getInstance().setChosenSkinImageId(allSkins.get(position).getImage());
            finish();
        }
        else{
            Toast.makeText(SkinsScreenActivity.this, "This skin is locked.[For now ;)]", Toast.LENGTH_LONG).show();
        }
    }

    public void updateSkinsStatus(){
        int currentNumOfWins = User.getInstance().getNumOfWins();
        for(int i = 0; i < this.allSkins.size(); i++){
            if(currentNumOfWins >= i){
                this.allSkins.get(i).setUnlocked(true);
            }
        }
    }
